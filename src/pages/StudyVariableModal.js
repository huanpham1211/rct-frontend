...
const StudyVariableModal = ({ studyId, onClose, onSuccess }) => {
  ...
  const fetchVariables = async () => {
    try {
      const res = await axios.get(`https://rct-backend-1erq.onrender.com/api/studies/api/studies/${studyId}/variables`, {
  ...
  const handleAdd = async () => {
    ...
    try {
      await axios.post(`https://rct-backend-1erq.onrender.com/api/studies/api/studies/${studyId}/variables`, newVar, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://rct-backend-1erq.onrender.com/api/studies/api/studies/variables/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
